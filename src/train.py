"""Базовое обучение ResNet18 на Train_1, валидация на Test_1.

Логирование в TensorBoard, артефакт модели загружается в S3 (MinIO).
"""
...
def main(params_path: str):
    ...
    log_dir = Path(paths["tensorboard_dir"]) / "train_base"
    ...
    print("\nEvaluating on Test_1 ...")
    te_loss, te_acc, te_f1, te_prec, te_rec = evaluate(
        model, test_loader, criterion, device, desc="Test_1"
    )
    print(
    ...
    local_ckpt = ckpt_dir / "resnet18_base.pt"
    torch.save(model.state_dict(), local_ckpt)
    print(f"saved local checkpoint: {local_ckpt}")

    s3_utils.upload_file(s3_cfg, str(local_ckpt), s3_cfg["base_model_key"])

    save_metrics_json(
        Path(paths["metrics_dir"]) / "train_base.json",
        {
            "train": last_train,
            "test": {
                "loss": te_loss,
